                        ...
                        <table id="myTable" class="table text-nowrap mb-0 align-middle">
                            <thead class="text-dark fs-4">
                                <tr>
                                    ...
                                    <th>
                                        <h6 class="fs-4 fw-semibold mb-0">Jumlah Pemasukan</h6>
                                    </th>
                                    <th>
                                        <h6 class="fs-4 fw-semibold mb-0">Jumlah Pengeluaran</h6>
                                    </th>
                                    <th>
                                        <h6 class="fs-4 fw-semibold mb-0">Jumlah Tanggungan</h6>
                                    </th>
                                    <th>
                            ...
                            <tbody>
                                @foreach ($pengajuans as $pengajuan)
                                    <tr>
                                        ...
                                        <td>Rp {{ number_format($pengajuan->jumlah_pemasukan, 0, ',', '.') }}</td>
                                        <td>Rp {{ number_format($pengajuan->jumlah_pengeluaran, 0, ',', '.') }}</td>
                                        ...
                                        <td>
                                            <span
                                                class="badge
        @if ($pengajuan->nasabah->bi_checking == 'Lancar') bg-success
        @elseif($pengajuan->nasabah->bi_checking == 'Dalam Pengawasan Khusus') bg-warning
        @elseif($pengajuan->nasabah->bi_checking == 'Kurang Lancar') bg-danger
        @elseif($pengajuan->nasabah->bi_checking == 'Diragukan') bg-danger
        @elseif($pengajuan->nasabah->bi_checking == 'Macet') bg-dark @endif
    ">
                                                {{ $pengajuan->nasabah->bi_checking }}
                                            </span>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            @if ($pengajuan->status === 'pending')
                                                <form action="{{ route('pengajuan.updateStatus', $pengajuan->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
                                        ...
                                        <td>
                                            @if ($pengajuan->status === 'pending')
                                                ...
                                                <form id="delete-form-{{ $pengajuan->id }}"
                                                    action="{{ route('pengajuan.destroy', $pengajuan->id) }}"
                                                    method="POST" style="display: inline;">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button class="btn bg-danger-subtle text-danger"
                                                        onclick="confirmDelete({{ $pengajuan->id }})"><i
