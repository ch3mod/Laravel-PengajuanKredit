...
class NasabahController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'bi_checking' => 'required|in:Lancar,Dalam Pengawasan Khusus,Kurang Lancar,Diragukan,Macet'
        ]);
        ...
        // Pengkondisian berdasarkan BI Checking
        if (in_array($nasabah['bi_checking'], ['Lancar', 'Dalam Pengawasan Khusus'])) {
            // Jika BI Checking "Lancar" atau "Dalam Pengawasan Khusus"
            return redirect()->route('nasabah.dashboard')
                ->with('success', 'Data nasabah berhasil disimpan');
        } else {
            // Jika BI Checking "Kurang Lancar", "Diragukan", atau "Macet"
            return redirect()->route('nasabah.dashboard')
                ->with('success', 'Bi Checking Tidak Memenuhi Syarat. Data nasabah tetap disimpan.');
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        // Cek apakah nasabah memiliki pengajuan kredit
        $hasPengajuanKredit = $nasabah->pengajuanKredit()->exists();

        // Jika nasabah memiliki pengajuan kredit, bi_checking tidak dapat diubah
        if ($hasPengajuanKredit) {
            // Update hanya data lain tanpa mengubah bi_checking
            $nasabah->update([
                'nama' => $request->nama,
                'alamat' => $request->alamat,
                'no_telepon' => $request->no_telepon,
            ...
            return redirect()->route('nasabah.dashboard')
                ->with('warning', 'Nasabah memiliki pengajuan kredit, BI Checking tidak dapat diubah.');
        }
        ...
        if (in_array($nasabah['bi_checking'], ['Lancar', 'Dalam Pengawasan Khusus'])) {
            ...
            // Jika BI Checking "Kurang Lancar", "Diragukan", atau "Macet"
            return redirect()->route('nasabah.dashboard')
                ->with('warning', 'Bi Checking Tidak Memenuhi Syarat. Data nasabah tetap diperbarui.');
        }
